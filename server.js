...
app.post("/show-linha", async (req, res) => {
  try {
    ...
    const LinhaDigitavelStg = await buscaRelacaoRecibo(dados);
    let encryptedData = cipher.update(LinhaDigitavelStg, "utf8", "hex");
    encryptedData += cipher.final("hex");
    console.log(encryptedData);
    res.redirect(`/show-linha?linhaValue=${encryptedData}`);
  } catch (error) {
...
app.get("/show-linha", async (req, res) => {
  try {
    const LinhaDigitavelStg = req.query.linhaValue;
    const decipher = crypto.createDecipher("aes-256-cbc", process.env.SECRECY);
    let decryptedData = decipher.update(LinhaDigitavelStg, "hex", "utf8");
    decryptedData += decipher.final("utf8");
    res.render(__dirname + "/public/linhaDigitavel.ejs", {
      linhaDigitavelStg: decryptedData,
    });
  } catch (error) {
    console.error("Erro ao renderizar:", error);
    res.status(500).send("Erro ao renderizar a página");
    ...
    .then((result) => {
      return new Promise((resolve, reject) => {
        parseString(result, function (err, resultDataJson) {
          ...
          resolve(LinhaDigitavelStg); // saida da linha digitavel
        });
      });
    })
    .catch((error) => console.log("error", error));
